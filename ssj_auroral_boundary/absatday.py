...
class absatday(object):
    ...
    imgdir : str
        Path to write image files to if make_plot is True or plot_failed is
        True (see constructor for defualt behavior and fallbacks)
    make_plot : bool
    ...
    def __init__(self, cdffile, imgdir=None, make_plot=True, plot_failed=False,
        ...
        csvdir : str, optional
            ...
            If still fails raises RuntimeError
                **kwargs : dict, optional
                    Additional keyword arguements accepted by abcvs
        ...
        if 'dmsp' in cdffile:
            ...
            self.log.info("Satellite number determined to be %d"
                          % (self.satnum))
        else:
    ...
    def simple_passes(self,latitude):
        ...
        for k in range(1,npts):
            ...
            if latitude[k-1] < 0. and latitude[k] >= 0.:
                ...
                self.log.info("Entered Northern Hemisphere: ind:%d,lat:%.3f"
                              % (k,latitude[k]) )
            elif latitude[k-1] > 0. and latitude[k] <= 0.:
                entered_south.append(k)
                self.log.info("Entered Southern Hemisphere: ind:%d,lat:%.3f"
                              % (k,latitude[k]))
