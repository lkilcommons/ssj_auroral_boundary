...
def dmsp_spectrogram(times, flux, channel_energies=None, lat=None, lt=None,
	...
	ax_cb : None or colorbar axis reference, optional
		Allows caller to specify axis for spectrogram color bar; helpful for stackplot.
		If 'ax' is specified then so should 'ax_cb'.
	...
	Since DMSP SSJ channels are log-linearly spaced, the bins widths are taken to be log-constant
	and the bins are placed symmetric about the channel center energies. This is probably not exactly
	correct since the actual instrument response/sensitivity functions are likely more linear than
	log linear.
	Recall that channels are listed as [30,000 eV to 30 eV] in reverse order.
	"""
	...
	mappable = ax.pcolormesh(T,CH_E,masked_flux.transpose(),cmap=cmap, norm=LogNorm(vmin=z_min, vmax=z_max))
	...
	if label_it and isinstance(times[0], dt.datetime):
		...
		if  plotwidth_m <= 10.: #if the plot width is less than 10 minutes tick mark every minute
			majloc = mpldates.MinuteLocator(interval=1)
		elif  plotwidth_m <= 30.: #if the plot width is less than 1/2 hour tick mark every 5 minutes
			majloc = mpldates.MinuteLocator(interval=5)
		elif  plotwidth_h <= 1: #if the plot width is less than 1 hour, but more than 30 minutes, tick mark every 10 minutes
			majloc = mpldates.MinuteLocator(interval=10)
		elif plotwidth_h <= 3: #if less than 3 hours, but more than 1 use every 15 minutes
			majloc = mpldates.MinuteLocator(interval=15)
		elif plotwidth_h <= 5: #if less than 5 hours, but more than 3 use every half hour
		...
		#This is slow and throws errors if used with pcolor, used pcolormesh instead
		#ax.set_yscale('log')
